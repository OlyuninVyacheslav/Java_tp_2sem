...
public class Main
{
    public static void main(String[] args) {
        ...
        try {
            System.out.println("stringArray3");
    ...
    private static int transformAndSum (String[][] in) throws MyArraySizeException, MyArrayDataException {
        ...
        for (int i = 0; i < in.length; i++) {
            for (int j = 0; j < in[i].length; j++) {
                try {
                    sum += Integer.valueOf(in[i][j]);
                } catch (NumberFormatException e) {
                    throw new MyArrayDataException(String.format("В позиции [%d][%d] исходного массива находится не целое число '%s'.", i+1, j+1, in[i][j]));
                }
            }
        }
